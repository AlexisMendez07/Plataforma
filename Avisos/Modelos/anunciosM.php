...
class AnunciosM extends ConexionBD
{
    ...
    static public function ObtenerTodoM($datos,$tablaBD)
    {
        try {
            ...
            $pdo = ConexionBD::cBD()->prepare("SELECT tipo, titulo, contenido, fecha FROM $tablaBD ORDER BY fecha DESC LIMIT :inicio,:fin ");
    ...
    static public function ObtenerAnunciosM($datos,$tablaBD){
        try {
            //Se hace la consulta usando limit
            $pdo = ConexionBD::cBD()->prepare("SELECT tipo, titulo, contenido, fecha FROM $tablaBD WHERE tipo = :tipo ORDER BY fecha DESC LIMIT :inicio,:fin ");
